...
export default function EventCard({ event, layout = 'grid' }: EventCardProps) {
  ...
  if (layout === 'list') {
    ...
    return (
      <Link
        to={`/events/${event.id}`}
        className="flex items-center gap-3 bg-gray-900 border border-gray-800 rounded-xl px-3 py-2.5 hover:border-indigo-600 transition-colors group"
      >
        <div className="flex flex-col items-center justify-center w-10 shrink-0 text-center">
          <span className="text-[10px] uppercase tracking-wider text-indigo-400 font-medium leading-none">{month}</span>
